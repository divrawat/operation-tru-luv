
// export const DOMAIN = 'http://localhost:3000';
export const DOMAIN = 'https://operationtrulove.in.net';
export const DOMAIN_NAME = 'operationtrulove.in.net';
export const IMAGES_SUBDOMAIN = 'https://images.mangachimp.com/operation-true-love';

export const MANGA_DESCRIPTION = `<a href="${DOMAIN}"><b><u>Operation True Love</u></b></a> is a webtoon about Seojin, a high school student who comes up with a plan to win the affection of her crush, Sunjin. Alongside her friends, Seojin faces a series of humorous and touching moments as she attempts to make her romantic plan succeed. The story delves into themes such as love, friendship, and personal growth, capturing the ups and downs of teenage life.`;




export const MANGA_NAME = 'Operation True Love';
export const MANGA_AUTHOR = 'kkokkalee';
export const MANGA_RELEASE = '2019';
export const MANGA_STATUS = 'Ongoing';
export const MANGA_ARTIST = 'kkokkalee';
export const MANGA_STUDIO = 'None';
export const MANGA_GENRE = 'Drama, Romance, Comedy';

export const CHAPTER_PREFIX = 'chapter';
export const IMAGE_PREFIX = 'images/operation-true-love';
export const NEXT_PREVIOUS_PREFIX = 'chapter';
export const URL_PREFIX = 'chapter';

export const AUTHOR_PAGE = `${DOMAIN}/operation-true-love-team`;
export const LOGO_URL = `${DOMAIN}/logo.webp`;
export const COVER_IMG = `${DOMAIN}/cover.webp`;
export const BEHIND_COVER_IMG = "https://static1.srcdn.com/wordpress/wp-content/uploads/2024/01/operation-true-love-s-three-main-cahracters-on-a-bench.jpg";


// Navbar
export const logo = <img src={`${LOGO_URL}`} alt="Logo" className="h-[85px] w-[145px] mr-5 md:ml-0 ml-4 md:pb-2 md:my-0 my-1" />

export const APP_NAME = 'Operation True Love';
export const NavbarName = "Operation True Love";

export const APP_DESCRIPTION = `Read ${MANGA_NAME} manhwa or manga all chapters online. Seojin who devises a plan to win the heart of her crush, Sunjin. With the help of her friends, Seojin navigates through various comedic and heartfelt situations as she tries to execute her romantic strategy. The story explores themes of friendship, love, and self-discovery amidst the challenges of adolescence.`;

export const DISQUS_SHORTNAME = "my-cms-7";


export const MANGA_SUMMARY = [
    {
        id: 1,
        content: "Operation True Love is a heartwarming webtoon that follows Seojin, a typical high schooler with a crush on Sunjin, the school's popular boy. Determined to win his affection, Seojin devises a plan called Operation True Love, with the help of her close-knit group of friends."
    },
    {
        id: 2,
        content: "Each episode is a mix of humor and touching moments as Seojin navigates the awkwardness of teenage romance. From funny mishaps to emotional growth, she learns not just about love, but also about friendship and self-confidence. Her friends play key roles, offering support and lighthearted moments throughout her journey."
    },
    {
        id: 3,
        content: "As the story unfolds, Seojin discovers the importance of being true to herself, learning valuable lessons about love, identity, and growing up. Operation True Love is a charming blend of comedy and emotion, capturing the relatable challenges of first crushes and high school life. It's perfect for readers who enjoy lighthearted romance with a focus on personal growth and the joys of friendship."
    },
    {
        id: 4,
        content: "More than just a romance, Operation True Love delves into themes of friendship, self-discovery, and growing up. Seojin's friends play an essential role in the story, adding comic relief and emotional support as they guide her through the ups and downs of teenage life. Whether it's giving her romantic advice or simply standing by her side, they contribute to her personal growth in meaningful ways."
    },
    {
        id: 5,
        content: "As Seojin embarks on her romantic quest, she gradually realizes that love isn't just about winning someone over—it's about learning to love yourself and others. Along the way, she begins to understand the importance of self-confidence and staying true to who she is, rather than trying to be someone she's not. The webtoon beautifully balances humor with sincere emotional depth, making it both entertaining and thought-provoking."
    },
    {
        id: 6,
        content: "By the end of the series, Operation True Love not only tells the story of Seojin's romantic pursuit but also celebrates the powerful bonds of friendship and the lessons learned through both success and failure. Seojin's journey is a relatable one for readers of all ages, especially for those who remember the highs and lows of their teenage years. With its lovable characters and engaging plot, the webtoon strikes a perfect balance between lighthearted fun and the deeper struggles of growing up, making it a must-read for fans of romantic comedies and coming-of-age stories alike."
    }
];




export const navLinks = [
    { text: 'Home', href: `${DOMAIN}` },
    { text: 'DMCA', href: `${DOMAIN}/dmca` },
    { text: 'Terms & Conditions', href: `${DOMAIN}/terms-and-conditions` },
];

export const FooterLinks = [
    // { text: 'About', href: `${DOMAIN}/about` },
    { text: 'Contact', href: `${DOMAIN}/contact` },
    { text: 'Disclaimer', href: `${DOMAIN}/disclaimer` },
    // { text: 'Privacy Policy', href: `${DOMAIN}/privacy-policy` },
    // { text: 'Terms & Conditions', href: `${DOMAIN}/terms-and-conditions` }
];



export const RelatedMangaLinks = [
    {
        imageUrl: `${DOMAIN}/related/whomademeaprincess.webp`,
        title: 'Who Made Me A Princess',
        link: 'https://www.whomademeaprincess.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/warriorhighschool.webp`,
        title: 'Warrior High School',
        link: 'https://www.warriorhighschool.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/thislifestartsasachildactor.webp`,
        title: 'This Life Starts As a Child Actor',
        link: 'https://www.thislifestartsasachildactor.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/thevillaninsfavouriteisme.webp`,
        title: 'The Villanins Favourite is Me',
        link: 'https://www.thevillaninsfavouriteisme.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/thegreatestestatedeveloper.webp`,
        title: 'The Greatest Estate Developer',
        link: 'https://www.thegreatestestatedeveloper.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/thedivorceesdessertcafe.webp`,
        title: 'The Divorcees Dessert Cafe',
        link: 'https://www.thedivorceesdessertcafe.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/thebeginningaftertheend.webp`,
        title: 'The Beginning After The End',
        link: 'https://www.thebeginningaftertheend.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/talentedbabysquirrel.webp`,
        title: 'Talented Baby Squirrel',
        link: 'https://www.talentedbabysquirrel.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/solomaxlevelnewbie.webp`,
        title: 'Solo Max Level Newbie',
        link: 'https://www.solomaxlevelnewbie.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/secondliferanker.webp`,
        title: 'Second Life Ranker',
        link: 'https://www.secondliferanker.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/operationtruelove.webp`,
        title: 'Operation True Love',
        link: 'https://www.operationtruelove.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/omniscientreadersviewpoint.webp`,
        title: 'Omniscient Readers Viewpoint',
        link: 'https://www.omniscientreadersviewpoint.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/myschoollifepretendingtobeaworthlessperson.webp`,
        title: 'My School Life Pretending to be a Worthless Person',
        link: 'https://www.myschoollifepretendingtobeaworthlessperson.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/kallithechampion.webp`,
        title: 'Kalli The Champion',
        link: 'https://www.kallithechampion.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/ishallmasterthisfamily.webp`,
        title: 'I Shall Master This Family',
        link: 'https://www.ishallmasterthisfamily.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/iobtainedamythicitem.webp`,
        title: 'I Obtained A Mythic Item',
        link: 'https://www.iobtainedamythicitem.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/goldenlightgratiathechildlovedbygod.webp`,
        title: 'Golden Light Gratia The Child Loved By God',
        link: 'https://www.goldenlightgratiathechildlovedbygod.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/disastrouscatastrophicnecromancer.webp`,
        title: 'Disastrous Catastrophic Necromancer',
        link: 'https://www.disastrouscatastrophicnecromancer.in.net',
    },
    {
        imageUrl: `${DOMAIN}/related/adoptedbyamurderousdukefamily.webp`,
        title: 'Adopted by a Murderous Duke Family',
        link: 'https://www.adoptedbyamurderousdukefamily.in.net',
    },
];



export const chaptersData = [
    {
        "chapterNumber": "0",
        "numImages": 23
    },
    {
        "chapterNumber": "1",
        "numImages": 32
    },
    {
        "chapterNumber": "2",
        "numImages": 53
    },
    {
        "chapterNumber": "3",
        "numImages": 35
    },
    {
        "chapterNumber": "4",
        "numImages": 56
    },
    {
        "chapterNumber": "5",
        "numImages": 45
    },
    {
        "chapterNumber": "6",
        "numImages": 3
    },
    {
        "chapterNumber": "7",
        "numImages": 15
    },
    {
        "chapterNumber": "8",
        "numImages": 2
    },
    {
        "chapterNumber": "9",
        "numImages": 2
    },
    {
        "chapterNumber": "10",
        "numImages": 2
    },
    {
        "chapterNumber": "11",
        "numImages": 3
    },
    {
        "chapterNumber": "12",
        "numImages": 3
    },
    {
        "chapterNumber": "13",
        "numImages": 18
    },
    {
        "chapterNumber": "14",
        "numImages": 12
    },
    {
        "chapterNumber": "15",
        "numImages": 12
    },
    {
        "chapterNumber": "16",
        "numImages": 6
    },
    {
        "chapterNumber": "17",
        "numImages": 16
    },
    {
        "chapterNumber": "18",
        "numImages": 10
    },
    {
        "chapterNumber": "19",
        "numImages": 16
    },
    {
        "chapterNumber": "20",
        "numImages": 22
    },
    {
        "chapterNumber": "21",
        "numImages": 31
    },
    {
        "chapterNumber": "22",
        "numImages": 5
    },
    {
        "chapterNumber": "23",
        "numImages": 16
    },
    {
        "chapterNumber": "24",
        "numImages": 4
    },
    {
        "chapterNumber": "25",
        "numImages": 7
    },
    {
        "chapterNumber": "26",
        "numImages": 7
    },
    {
        "chapterNumber": "27",
        "numImages": 21
    },
    {
        "chapterNumber": "28",
        "numImages": 6
    },
    {
        "chapterNumber": "29",
        "numImages": 9
    },
    {
        "chapterNumber": "30",
        "numImages": 12
    },
    {
        "chapterNumber": "31",
        "numImages": 9
    },
    {
        "chapterNumber": "32",
        "numImages": 14
    },
    {
        "chapterNumber": "33",
        "numImages": 13
    },
    {
        "chapterNumber": "34",
        "numImages": 2
    },
    {
        "chapterNumber": "35",
        "numImages": 2
    },
    {
        "chapterNumber": "36",
        "numImages": 2
    },
    {
        "chapterNumber": "37",
        "numImages": 4
    },
    {
        "chapterNumber": "38",
        "numImages": 26
    },
    {
        "chapterNumber": "39",
        "numImages": 29
    },
    {
        "chapterNumber": "40",
        "numImages": 24
    },
    {
        "chapterNumber": "41",
        "numImages": 31
    },
    {
        "chapterNumber": "42",
        "numImages": 17
    },
    {
        "chapterNumber": "43",
        "numImages": 29
    },
    {
        "chapterNumber": "44",
        "numImages": 30
    },
    {
        "chapterNumber": "45",
        "numImages": 25
    },
    {
        "chapterNumber": "46",
        "numImages": 28
    },
    {
        "chapterNumber": "47",
        "numImages": 27
    },
    {
        "chapterNumber": "48",
        "numImages": 17
    },
    {
        "chapterNumber": "49",
        "numImages": 28
    },
    {
        "chapterNumber": "50",
        "numImages": 19
    },
    {
        "chapterNumber": "51",
        "numImages": 21
    },
    {
        "chapterNumber": "52",
        "numImages": 28
    },
    {
        "chapterNumber": "53",
        "numImages": 18
    },
    {
        "chapterNumber": "54",
        "numImages": 23
    },
    {
        "chapterNumber": "55",
        "numImages": 24
    },
    {
        "chapterNumber": "56",
        "numImages": 26
    },
    {
        "chapterNumber": "57",
        "numImages": 22
    },
    {
        "chapterNumber": "58",
        "numImages": 14
    },
    {
        "chapterNumber": "59",
        "numImages": 16
    },
    {
        "chapterNumber": "60",
        "numImages": 15
    },
    {
        "chapterNumber": "61",
        "numImages": 14
    },
    {
        "chapterNumber": "62",
        "numImages": 15
    },
    {
        "chapterNumber": "63",
        "numImages": 14
    },
    {
        "chapterNumber": "64",
        "numImages": 16
    },
    {
        "chapterNumber": "65",
        "numImages": 20
    },
    {
        "chapterNumber": "66",
        "numImages": 18
    },
    {
        "chapterNumber": "67",
        "numImages": 19
    },
    {
        "chapterNumber": "68",
        "numImages": 15
    },
    {
        "chapterNumber": "69",
        "numImages": 14
    },
    {
        "chapterNumber": "70",
        "numImages": 18
    },
    {
        "chapterNumber": "71",
        "numImages": 20
    },
    {
        "chapterNumber": "72",
        "numImages": 17
    },
    {
        "chapterNumber": "73",
        "numImages": 19
    },
    {
        "chapterNumber": "74",
        "numImages": 18
    },
    {
        "chapterNumber": "75",
        "numImages": 16
    },
    {
        "chapterNumber": "76",
        "numImages": 17
    },
    {
        "chapterNumber": "77",
        "numImages": 23
    },
    {
        "chapterNumber": "78",
        "numImages": 23
    },
    {
        "chapterNumber": "79",
        "numImages": 21
    },
    {
        "chapterNumber": "80",
        "numImages": 14
    },
    {
        "chapterNumber": "81",
        "numImages": 18
    },
    {
        "chapterNumber": "82",
        "numImages": 14
    },
    {
        "chapterNumber": "83",
        "numImages": 35
    },
    {
        "chapterNumber": "84",
        "numImages": 15
    },
    {
        "chapterNumber": "85",
        "numImages": 20
    },
    {
        "chapterNumber": "86",
        "numImages": 12
    },
    {
        "chapterNumber": "87",
        "numImages": 14
    },
    {
        "chapterNumber": "88",
        "numImages": 12
    },
    {
        "chapterNumber": "89",
        "numImages": 17
    },
    {
        "chapterNumber": "89a",
        "numImages": 2
    },
    {
        "chapterNumber": "89b",
        "numImages": 11
    },
    {
        "chapterNumber": "90",
        "numImages": 11
    },
    {
        "chapterNumber": "91",
        "numImages": 11
    },
    {
        "chapterNumber": "92",
        "numImages": 13
    },
    {
        "chapterNumber": "93",
        "numImages": 12
    },
    {
        "chapterNumber": "94",
        "numImages": 12
    },
    {
        "chapterNumber": "95",
        "numImages": 11
    },
    {
        "chapterNumber": "96",
        "numImages": 14
    },
    {
        "chapterNumber": "97",
        "numImages": 6
    },
    {
        "chapterNumber": "98",
        "numImages": 11
    },
    {
        "chapterNumber": "99",
        "numImages": 4
    },
    {
        "chapterNumber": "100",
        "numImages": 4
    },
    {
        "chapterNumber": "101",
        "numImages": 6
    },


    {
        "chapterNumber": "102",
        "numImages": 6
    },
    {
        "chapterNumber": "103",
        "numImages": 7
    },
    {
        "chapterNumber": "104",
        "numImages": 6
    },
    {
        "chapterNumber": "105",
        "numImages": 10
    },
    {
        "chapterNumber": "106",
        "numImages": 22
    },

]